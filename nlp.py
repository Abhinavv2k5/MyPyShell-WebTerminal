...
# Load .env (locally only, .env should be in .gitignore)
load_dotenv()

# Read Hugging Face token from environment, NO default token here
...
def simple_rule_parse(text):
    ...
    for cmd in commands_list:
        ...
        if folder_match:
            names = re.split(r"\s*,\s*|\s+", folder_match.group(2).strip())
        ...
        if file_match:
            names = re.split(r"\s*,\s*|\s+", file_match.group(2).strip())
        ...
        if m2:
            targets = re.sub(r"\s*,\s*", " ", m2.group(2).strip())
        ...
        if m3:
            parsed_commands.append(f"cd {m3.group(2)}")
            continue
        ...
        if m4:
            parsed_commands.append(f"cat {m4.group(2)}")
            continue
